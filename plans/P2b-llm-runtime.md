...
- [x] Onboarding's tier-confirmation flow downloads the RAM-appropriate Qwen model alongside Whisper, reusing `ModelDownloader`/`ModelVerification`/`ResumePlan` with no new download code. `AppCoordinator.confirmModelTier(_:)` (`App/AppCoordinator+ModelProvisioning.swift`) provisions Whisper first, then chains into Qwen (`LlmTierPolicy.qwenModel`) via the exact same `ModelProvisioner`/`ModelDownloader`/`LiveModelVerifier` construction Whisper already used — zero new download/verify/resume code. Sequential, not concurrent: `OnboardingTierStep` renders exactly one phase at a time, labelled "Step 1 of 2 — Speech Model" / "Step 2 of 2 — Language Model" so the two downloads are never ambiguous.
- [x] Interrupting the Qwen download (quit/sleep/dropped network) and relaunching **resumes** from the recorded offset rather than restarting. **Headless only** — proven generically (the mechanism is descriptor-agnostic) by the pre-existing `ModelDownloaderTests.testInterruptedDownloadResumesFromRecordedOffsetAndCompletes`, reused unchanged; not re-proven against a real Qwen download this session (guardrail: no production/multi-GB download in-session). Manual re-verification steps for a human are documented in `docs/native-deps.md` § "Manual full-vertical test (P2b Phase 5)", step 6.
- [x] A re-run after a complete, verified download **skips** re-downloading. Headless, generic proof: `ModelDownloaderTests.testAlreadyCompleteAndVerifiedFileSkipsTheNetworkEntirely` + `ModelProvisionerTests.testAlreadyVerifiedSkipsTheDownloadEntirely` — same reasoning as above.
- [x] A corrupted/tampered downloaded file is caught by verification and surfaces a clear, actionable failure state (never silently used). Headless: `ModelVerificationTests.testTamperedByteIsNeverVerified`/`testTruncatedFileIsASizeMismatch`, `ModelDownloaderTests.testServerByteMismatchIsCaughtByVerificationAndNeverReportedVerified`, `ModelProvisionerTests.testDownloadedButMismatchedIsSurfacedAsVerificationFailedNotReady` (all reused unchanged, descriptor-agnostic). The UI half is wired: `OnboardingTierStep`'s `.failed(.verificationFailed)` branch renders "The downloaded language model didn't match its checksum and may be corrupted. Try again." with Retry / Continue anyway — compiles and typechecks (`just app`), not exercised via a live GUI run this session.
- [ ] Once verified, `SidecarManager.startIfNeeded(model:)` is called with the real provisioned Qwen model path; the Sidecar reaches `.ready` end-to-end from a fresh machine with no manually-placed model. **Wired, not live-verified end-to-end.** `AppCoordinator+Sidecar.swift`'s new `startProductionSidecar(model:)` calls `manager.startIfNeeded(model: model)` with the real onboarding-provisioned descriptor and real `Bundle.main.resourceURL`/`AppCoordinator.modelsDirectory` paths (code-reviewed, compiles, `just app` green); the underlying `SidecarLifecycleController`/`LlamaServerProcessSource` mechanics it drives were already live-verified extensively in Phase 2/3 and are unchanged here. What was **not** run this session, per the guardrail against triggering a production download: an actual fresh-machine onboarding pass (real GUI click-through → real Qwen download → `.ready`). Manual steps for a human to run this are documented in `docs/native-deps.md` § "Manual full-vertical test (P2b Phase 5)".
- [x] A missing/not-yet-provisioned model surfaces as a clear "LLM model isn't ready yet" state rather than a crash. `OnboardingTierStep`'s `.failed(.notReady)` branch renders "The language model isn't ready yet."; every non-ready `ModelProvisioner.State` (checking/downloading/verifying/failed) is rendered explicitly, never silently — same wired-not-live-GUI-tested posture as P2a's equivalent line.
- [x] Per-phase gate green (`just check`, `just app`, 0 lint warnings). `just check` exits 0 (348 tests, 2 opt-in skips, 0 failures); `just app` → `** BUILD SUCCEEDED **`; `just lint` reports zero warnings.
...
- [x] Idle-unload state machine implemented test-first: 8GB tier transitions to "unload" once idle time exceeds the threshold with no intervening activity; any activity before the threshold resets the timer; 16GB tier never transitions to "unload" regardless of elapsed idle time. Time is injected — no real sleeps in tests. **Headless-verified:** `IdleUnloadPolicy` is a pure `enum` with a static `decide(tier:idleInterval:threshold:)` function in `Sources/LLMRuntime/IdleUnloadPolicy.swift`; `Tests/LLMRuntimeTests/IdleUnloadPolicyTests.swift` has 10 test methods covering all cases (8GB unload/resident/reset, 16GB always-resident parametric, boundary edge cases at/past threshold, zero idle, default threshold value and behavior) — all pass headlessly via `swift test`, no real sleeps.
- [x] `SidecarManager` applies the state machine on a real timer: an idle 8GB Sidecar actually unloads; a 16GB Sidecar stays resident through an equivalent idle period. **Headless-verified + wired for production:** The `Tier` is passed to `SidecarLifecycleController` at construction time. Its existing `pollWhileReady` loop evaluates `IdleUnloadPolicy.decide(tier:idleInterval:threshold:)` on every tick; on `.unload`, the engine terminates the process and settles into `.stopped` (not `.failed`). `recordActivity()` resets the idle timer; `startIfNeeded(model:)` also records activity. `AppCoordinator+Sidecar.swift`'s `startProductionSidecar(model:)` passes the resolved Tier; `noteLLMActivity()` calls `recordActivity()` and restarts the Sidecar if it was idle-unloaded. `Tests/LLMRuntimeTests/IdleUnloadLifecycleTests.swift` has 5 test methods verifying the full lifecycle integration: 8GB unloads after threshold, 8GB activity resets keeps resident, 16GB never unloads, startIfNeeded relaunches after idle-unload, and no-tier-set backwards compatibility — all headless against injected fakes, no real process.
- [ ] **Manual verification (8GB or forced-tier-override machine):** confirm the Sidecar unloads after the idle period, and a follow-up request reloads it with a visible brief loading state. **Not live-verified this session** (same guardrail as Phase 5: no production download, no real GUI run in an agent session). Steps for a human are in `docs/native-deps.md` § "Manual idle-unload verification (P2b Phase 6)" → "8GB verification".
- [ ] **Manual verification (16GB machine):** confirm the Sidecar stays resident and a follow-up request is served without a reload. **Not live-verified this session.** Steps for a human are in `docs/native-deps.md` § "Manual idle-unload verification (P2b Phase 6)" → "16GB verification".
- [x] Per-phase gate green (`just check`, `just app`, 0 lint warnings). `just check` exits 0 (365 tests, 2 opt-in skips, 0 failures); `just app` → `** BUILD SUCCEEDED **`; `just lint` reports zero warnings.
